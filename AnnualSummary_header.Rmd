...
aliens = rbind(dhs., 
               ...
               refugees %>% rename(Country = presentcountry) ) %>% as.data.frame() %>%
     filter(year >= year.start & year <= year.end) %>%
...
yearly = aliens %>% group_by(year, AlienType, Continent, Country) %>% summarise(count = sum(count))
...
```{r tornado_plot_refugees, fig.height = 2, dpi=300, fig.cap="Age and sex distribution of newly arriving **refguees**" }
...
```{r tornado_plot_immigrants, fig.height = 2, dpi=300, fig.cap="Age and sex distribution of **newly arriving immigrants**" }

text.scale = 3

# av %>% count(AlienType)

data = av %>% filter(AlienType %in% 'New Immigrant Arrivals')
max.count=max(abs(data$Arrivals))
...
```{r tornado_plot_aos, fig.height = 2, dpi=300, fig.cap="Age and sex distribution of those **adjusting status to immigrant**" }
...
.countries = top_refugee_presentcountries[, "Country of Origin"]
.nationalities = top_refugee_nationalities[, "Nationality"]


# kable(refs[1:20, ] %>% mutate(Arrivals = comma(Arrivals)), align = 'r' )
